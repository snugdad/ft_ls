...
/*   Updated: 2018/05/31 14:37:31 by egoodale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** display.c
*/
int						display_all(t_file *begin, int flags, int first,
									int n);
static void				display_full_path(char *full_path, int nof, int *first);
static t_file			*read_folder(char path[PATH_MAX], char *name,
									int options);
...
static void				populate_chmod(char chmod[12], int mode,
							char path[PATH_MAX]);
int						display_list_items(t_file *file, size_t size[7],
							int flags);
int						get_vector_size(t_file *file, size_t size[7],
							int *blocks);
...
static int				lst_column(t_file *lst, t_coord rc, t_coord max,
									int flags);
...
void					front_back_split(t_file *source, t_file **front_ref,
										t_file **back_ref);
t_file					*sorted_merge(t_file *a, t_file *b, int key,
										t_bool timesort_delta);
